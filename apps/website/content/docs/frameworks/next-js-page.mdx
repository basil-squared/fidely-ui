...
It comes preconfigured with Panda CSS, Ark UI, Fidely UI, and theme support.
...
> When using libraries like **next-themes**, it is recommended to add the  
> **suppressHydrationWarning** prop to the `Html` element in **\_document.tsx**  
> to prevent hydration mismatch warnings.

## Configure **\_document.tsx** (for next-themes)

Update **pages/\_document.tsx** as shown below:
...
This prevents hydration warnings caused by theme mismatches when **next-themes**
switches between light and dark modes on the client.
