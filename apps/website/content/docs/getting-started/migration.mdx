...
Remove the unused packages **`@fidely-ui/styled-system`**

```bash
npm uninstall @fidely-ui/styled-system
```

Install updated versions of the packages: **`@fidely-ui/react`** and **`@fidely-ui/panda-preset`**.

```bash
npm install @fidely-ui/react@latest @fidely-ui/panda-preset@latest
```
...
And your **tsconfig.json** must include:
...
### Vite Users: Alias Configuration (Required)
...
**`Next.js`** automatically respects **`tsconfig.json`** paths, so no extra config is needed as long as:

- **`baseUrl`** is set to "."

- **`styled-system/`** exists in **paths**
...
export default defineConfig({
  ...
  include: [
    './node_modules/@fidely-ui/react/**/*.{js,jsx,ts,tsx}', // include this
  ],
})
```
