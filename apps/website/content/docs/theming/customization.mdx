...
Fidely UI is built on top of **Panda CSS**, so you can customize it at the token, theme, and component levels.
By default, **Fidely UI** uses both a neutral color for **grayColor** and an **accentColor**.

You can choose a **neutral (gray color)** and an **accent color** for your brand.
...
**Fidely UI** uses **[Radix Colors](https://www.radix-ui.com/colors/custom)** by default, so you can base your palette on their design system.
...
Example for extending the **Button** recipe:
...
```ts
import { buttonRecipe } from './theme/recipes/button.ts'
...
## You can also add new variant or style props
...
Use the new **`primary`** variant in your button:
...
To make the newly variant as default instead of the solid
...
- Extend recipes or overriding base component styles for cleaner maintenance.
...
Extendable component **recipes** or **slotRecipes**
